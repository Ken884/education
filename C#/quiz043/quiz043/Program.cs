...
namespace quiz043
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            var orderGenrals = generals.OrderByDescending(x => x.DeadYear - x.BirthYear).Select(g => new {Name = g.Name, Age = (g.DeadYear - g.BirthYear) });

            foreach (var general in orderGenrals)
            {
                Console.WriteLine($"{general.Name}, 没時年齢{general.Age}歳");
            }
        }
    }
